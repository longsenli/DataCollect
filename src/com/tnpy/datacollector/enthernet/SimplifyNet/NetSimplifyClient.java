package com.tnpy.datacollector.enthernet.SimplifyNet;
 ...
 * 同步访问的网络客户端
 */
public class NetSimplifyClient extends NetworkDoubleBase<NetMessage, RegularByteTransform> {
	...
	OperateResultExOne<byte[]> temp = ReadFromServerBase(Protocol.ProtocolUserString, customer.get_CodeValue(),
		data);
	...
	return ReadFromServerBase(Protocol.ProtocolUserBytes, customer.get_CodeValue(), send);
    }
	...
	OperateResultExOne<byte[]> read = ReadFromCoreServer(Protocol.CommandBytes(headCode, customer, Token, send));
	if (!read.IsSuccess) {
	    return read;
	}

	if (read.Content.length < Protocol.HeadByteLength) {
	    read.IsSuccess = false;
	    read.Message = "length is not correct :" + read.Content.length;
	    return read;
	}

	byte[] headBytes = new byte[Protocol.HeadByteLength];
	byte[] contentBytes = new byte[read.Content.length - Protocol.HeadByteLength];

	System.arraycopy(read.Content, 0, headBytes, 0, Protocol.HeadByteLength);
	if (contentBytes.length > 0) {
	    System.arraycopy(read.Content, Protocol.HeadByteLength, contentBytes, 0,
		    read.Content.length - Protocol.HeadByteLength);
	}

	contentBytes = Protocol.CommandAnalysis(headBytes, contentBytes);
