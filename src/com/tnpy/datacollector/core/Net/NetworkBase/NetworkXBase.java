...
public class NetworkXBase extends NetworkBase {
	...
	thread = new Thread() {
	    ...
	    public void run() {
		while (true) {
		    OperateResultExOne<byte[]> readHeadBytes = Receive(session.getWorkSocket(),
			    Protocol.HeadByteLength);
			...
			byte[] content = Protocol.CommandAnalysis(head, readContent.Content);
	...
	// 数据处理
	send = Protocol.CommandBytes(headcode, customer, Token, send);
	...
	return SendBaseAndCheckReceive(socket, Protocol.ProtocolUserBytes, customer, send);
    }
	...
	return SendBaseAndCheckReceive(socket, Protocol.ProtocolUserString, customer, data);
    }
...
//    }
	...
	// if (timeout > 0) ThreadPool.QueueUserWorkItem( new WaitCallback(
	// ThreadPoolCheckTimeOut ), hslTimeOut );

	// 接收头指令
	OperateResultExOne<byte[]> headResult = Receive(socket, Protocol.HeadByteLength, timeout);
	...
	int contentLength = Utilities.getInt(headResult.Content, Protocol.HeadByteLength - 4);
	// 接收内容
	...
	// 返回成功信息
	OperateResult checkResult = SendLong(socket, Protocol.HeadByteLength + contentLength);
	if (!checkResult.IsSuccess)
	    return OperateResultExTwo.CreateFailedResult(checkResult);

	byte[] head = headResult.Content;
	byte[] content = contentResult.Content;
	content = Protocol.CommandAnalysis(head, content);
	...
	// 检查是否是字符串信息
	if (Utilities.getInt(receive.Content1, 0) != Protocol.ProtocolUserString) {
	    if (LogNet != null)
	...
	// 分析数据
	return OperateResultExTwo.CreateSuccessResult(Utilities.getInt(receive.Content1, 4),
		Utilities.byte2String(receive.Content2));
    }
	...
	// 检查是否是字节信息
	if (Utilities.getInt(receive.Content1, 0) != Protocol.ProtocolUserBytes) {
	    if (LogNet != null)
		LogNet.WriteError(toString(), StringResources.Language.CommandHeadCodeCheckFailed());
	    CloseSocket(socket);
...
//    }
     ...
     * @return 是否成功
     */
    protected OperateResult SendStream(Socket socket, InputStream stream, long receive,
	    ActionOperateExTwo<Long, Long> report, boolean reportByPercent) {
     ...
     * @return 结果类对象
     */
    protected OperateResult WriteStream(Socket socket, OutputStream stream, long totalLength,
	    ActionOperateExTwo<Long, Long> report, boolean reportByPercent) {
