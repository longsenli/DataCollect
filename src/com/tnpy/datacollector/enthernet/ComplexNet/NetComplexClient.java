...
//    }
	...
	if (ConnectFailedCount == 0) {
		...
		if (MessageAlerts != null)
		    MessageAlerts.Action("连接断开，等待" + count + "秒后重新连接");
		try {
		    Thread.sleep(1000);
		} catch (Exception ex) {
	    ...
	    if (MessageAlerts != null)
		MessageAlerts.Action("正在尝试第" + ConnectFailedCount + "次连接服务器...");
	}
    }
	...
	isConnecting = 0;
	if (LoginFailed != null)
	    LoginFailed.Action(ConnectFailedCount);
	if (LogNet != null)
	...
	if (protocol == HslProtocol.ProtocolCheckSecends) {
	    ...
	    // 接收到字节数据
	    if (AcceptByte != null)
		AcceptByte.Action(this, new NetHandle(customer), content);
	} else if (protocol == HslProtocol.ProtocolUserString) {
	    // 接收到文本数据
	    String str = Utilities.byte2String(content);
	    if (AcceptString != null)
		AcceptString.Action(this, new NetHandle(customer), str);
	}
    }
