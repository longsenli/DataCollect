package com.tnpy.datacollector.enthernet.ComplexNet;
...
import com.tnpy.datacollector.core.Types.ActionOperate;
import com.tnpy.datacollector.core.Types.ActionOperateExOne;
...
//    }
    ...
    public void ClientClose() {
        IsQuie = true;
        if (IsClientStart)
            SendBytes(session, Protocol.CommandBytes(Protocol.ProtocolClientQuit, 0, Token, null));
    ...
    public void Send(NetHandle customer, String str) {
        if (IsClientStart) {
            SendBytes(session, Protocol.CommandBytes(customer.get_CodeValue(), Token, str));
        }
    }
    ...
    public void Send(NetHandle customer, byte[] bytes) {
        if (IsClientStart) {
            SendBytes(session, Protocol.CommandBytes(customer.get_CodeValue(), Token, bytes));
        }
    }
    ...
    protected void DataProcessingCenter(AppSession session, int protocol, int customer, byte[] content) {
        if (protocol == Protocol.ProtocolCheckSecends) {
        ...
        } else if (protocol == Protocol.ProtocolClientQuit) {
            // 申请了退出
        } else if (protocol == Protocol.ProtocolUserBytes) {
            // 接收到字节数据
            if (AcceptByte != null) AcceptByte.Action(this, new NetHandle(customer), content);
        } else if (protocol == Protocol.ProtocolUserString) {
            // 接收到文本数据
    ...
    private void ThreadHeartCheck() {
        ...
        while (true) {
            ...
            if (!IsQuie) {
                ...
                try {
                    SendBytes(session, Protocol.CommandBytes(Protocol.ProtocolCheckSecends, 0, Token, send));
