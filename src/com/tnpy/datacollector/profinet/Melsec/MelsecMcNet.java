...
import com.tnpy.datacollector.core.IMessage.MelsecQnA3EBinaryMessage;
...
public class MelsecMcNet extends NetworkDeviceBase<MelsecQnA3EBinaryMessage, RegularByteTransform> {
	...
	if (errorCode != 0)
	    return new OperateResultExOne<>(errorCode, StringResources.Language.MelsecPleaseReferToManulDocument());
	...
	if (analysis.Content1.getDataType() == 0x00)
	    return new OperateResultExOne<boolean[]>(0, StringResources.Language.MelsecReadBitInfo());
	...
	if (ErrorCode != 0)
	    return new OperateResultExOne<byte[]>(ErrorCode,
     ...
     * @return 带有成功标志的指令数据
     */
    public static OperateResultExOne<byte[]> BuildReadCommand(String address, short length, byte networkNumber,
     ...
     * @return 结果
     */
    public static OperateResultExOne<byte[]> BuildWriteCommand(String address, byte[] value, byte networkNumber,
	    byte networkStationNumber) {
	OperateResultExTwo<MelsecMcDataType, Short> analysis = MelsecHelper.McAnalysisAddress(address);
	if (!analysis.IsSuccess)
	    return OperateResultExOne.CreateFailedResult(analysis);
