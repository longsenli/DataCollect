...
import com.tnpy.datacollector.StringResources;
import com.tnpy.datacollector.Utilities;
import com.tnpy.datacollector.basicframework.SoftBasic;
...
public class SiemensS7Net extends NetworkDeviceBase<S7Message, ReverseBytesTransform> {
    ...
    /**
     * 在客户端连接上服务器后，所做的一些初始化操作 -> Two handshake actions required after connecting to
     * the server
     * 
     * @param socket 网络套接字
     * @return 返回连接结果
	...
	if (read.Content.length >= 21 && read.Content[20] == 1) {
	    ...
	    return new OperateResultExOne<byte[]>(read.ErrorCode,
		    StringResources.Language.SiemensDataLengthCheckFailed());
	}
    }
	...
	if (read.Content.length >= 21 && read.Content[20] == length.length) {
		...
		if ((ii + 1) < read.Content.length) {
		    if (read.Content[ii] == (byte) 0xFF && read.Content[ii + 1] == 0x04) {
			// 有数据
    ...
    private byte[] plcHead1 = new byte[] { 0x03, 0x00, 0x00, 0x16, 0x11, (byte) 0xE0, 0x00, 0x00, 0x00, 0x01, 0x00,
	    (byte) 0xC0, 0x01, 0x0A, (byte) 0xC1, 0x02, 0x01, 0x02, (byte) 0xC2, 0x02, 0x01, 0x00 };
    private byte[] plcHead2 = new byte[] { 0x03, 0x00, 0x00, 0x19, 0x02, (byte) 0xF0, (byte) 0x80, 0x32, 0x01, 0x00,
	    0x00, 0x04, 0x00, 0x00, 0x08, 0x00, 0x00, (byte) 0xF0, 0x00, 0x00, 0x01, 0x00, 0x01, 0x01, (byte) 0xE0 };
    private byte[] plcOrderNumber = new byte[] { 0x03, 0x00, 0x00, 0x21, 0x02, (byte) 0xF0, (byte) 0x80, 0x32, 0x07,
	    0x00, 0x00, 0x00, 0x01, 0x00, 0x08, 0x00, 0x08, 0x00, 0x01, 0x12, 0x04, 0x11, 0x44, 0x01, 0x00, (byte) 0xFF,
	    0x09, 0x00, 0x04, 0x00, 0x11, 0x00, 0x00 };
    private SiemensPLCS CurrentPlc = SiemensPLCS.S1200;
    private byte[] plcHead1_200smart = new byte[] { 0x03, 0x00, 0x00, 0x16, 0x11, (byte) 0xE0, 0x00, 0x00, 0x00, 0x01,
	    0x00, (byte) 0xC1, 0x02, 0x10, 0x00, (byte) 0xC2, 0x02, 0x03, 0x00, (byte) 0xC0, 0x01, 0x0A };
    private byte[] plcHead2_200smart = new byte[] { 0x03, 0x00, 0x00, 0x19, 0x02, (byte) 0xF0, (byte) 0x80, 0x32, 0x01,
	    0x00, 0x00, (byte) 0xCC, (byte) 0xC1, 0x00, 0x08, 0x00, 0x00, (byte) 0xF0, 0x00, 0x00, 0x01, 0x00, 0x01,
	    0x03, (byte) 0xC0 };
	...
	try {
		...
		return new OperateResultExThree<Byte, Integer, Integer>(
    ...
    /**
     * 生成一个读取字数据指令头的通用方法 -> A general method for generating a command header to read
     * a Word data
     * 
     * @param address 起始地址，例如M100，I0，Q0，DB2.100 -> Start address, such as
     *                M100,I0,Q0,DB2.100
     ...
     * @return 携带有命令字节
     */
    public static OperateResultExOne<byte[]> BuildReadCommand(OperateResultExThree<Byte, Integer, Integer>[] address,
	    short[] length) {
	if (address == null)
     ...
     * 
     * @param address 起始地址，示例M100,I100,Q100,DB1.100 -> Start Address, example
     *                M100,I100,Q100,DB1.100
     ...
     * @return 指令
     */
    public static OperateResultExOne<byte[]> BuildWriteByteCommand(
	    OperateResultExThree<Byte, Integer, Integer> analysis, byte[] data) {
	if (data == null)
	    data = new byte[0];
